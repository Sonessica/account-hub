...
export default async function Dashboard({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <><Header/><main className="shell dashboard bento-dashboard">
    ...
    <section className="bento-toolbar"><form className="search"><Search size={18}/><input name="q" defaultValue={q} placeholder="搜索账号、平台、昵称、邮箱、UID…"/><input type="hidden" name="filter" value={filter}/></form><Link href="/accounts/new" className="button primary"><Plus size={16}/>添加账号</Link></section>
    ...
    {accounts.length?<BentoDashboard initialAccounts={accounts}/>:<div className="empty"><div>＋</div><h2>{q?"没有找到匹配账号":"还没有符合条件的账号"}</h2><p>搜索或筛选结果会自动重新组成紧凑 Bento。</p></div>}
  </main></>;
}
