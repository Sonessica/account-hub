...
export async function setup(form:FormData){if(await db.user.count())redirect("/login");const p=setupSchema.safeParse({username:text(form,"username"),password:text(form,"password")});if(!p.success)redirect("/setup?error=用户名至少3位，密码至少12位");const user=await db.user.create({data:{username:p.data.username,passwordHash:await argon2.hash(p.data.password,{type:argon2.argon2id})}});await db.auditLog.create({data:{action:"ADMIN_CREATED"}});await createSession(user.id);redirect("/");}
export async function login(form:FormData){const username=text(form,"username"),password=text(form,"password"),user=await db.user.findUnique({where:{username}});if(!user||!(await argon2.verify(user.passwordHash,password)))redirect("/login?error=用户名或密码错误");await createSession(user.id);await db.auditLog.create({data:{action:"LOGIN"}});redirect("/");}
export async function logout(){await destroySession();redirect("/login");}
export async function createAccount(form:FormData){await requireUser();const p=accountSchema.safeParse(values(form));if(!p.success)redirect("/accounts/new?error=请检查必填项、邮箱和网址格式");const{tags,...data}=p.data;const account=await db.account.create({data:{...data,tags:await tagLinks(tags??"")}});await db.auditLog.create({data:{action:"ACCOUNT_CREATE",accountId:account.id,detail:account.displayName}});redirect(`/accounts/${account.id}`);}
export async function updateAccount(id:string,form:FormData){await requireUser();const p=accountSchema.safeParse(values(form));if(!p.success)redirect(`/accounts/${id}/edit?error=请检查必填项、邮箱和网址格式`);const{tags,...data}=p.data;await db.account.update({where:{id},data:{...data,tags:{deleteMany:{},...(await tagLinks(tags??""))}}});await db.auditLog.create({data:{action:"ACCOUNT_UPDATE",accountId:id}});revalidatePath("/");redirect(`/accounts/${id}`);}
