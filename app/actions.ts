...
export async function updateAccountLayout(id:string,data:{bentoSize?:"compact"|"standard"|"wide"|"large";pinned?:boolean}){await requireUser();await db.account.update({where:{id},data});revalidatePath("/");}
export async function reorderAccounts(ids:string[]){await requireUser();if(ids.length>200)throw new Error("Too many accounts");await db.$transaction(ids.map((id,sortOrder)=>db.account.update({where:{id},data:{sortOrder}})));revalidatePath("/");}
